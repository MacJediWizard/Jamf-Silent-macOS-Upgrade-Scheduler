...
### Test 6: Command-Line Parameter Tests
**Goal**: Verify --show-config and --config= parameters work correctly

#### Test 6a: --show-config Parameter
- [ ] Run: `sudo /Library/Management/erase-install/erase-install-defer-wrapper.sh --show-config`
- [ ] Verify displays configuration source
- [ ] Verify shows all core settings
- [ ] Verify shows feature toggles
- [ ] Verify exits without running script
- [ ] Check output matches actual configuration
...
#### Test 6b: --config=/path/to/file.json Parameter
- [ ] Create test JSON: `/tmp/custom-test-config.json`
- [ ] Set unique values (e.g., `INSTALLER_OS=16`, `MAX_DEFERS=7`)
- [ ] Run: `sudo /Library/Management/erase-install/erase-install-defer-wrapper.sh --config=/tmp/custom-test-config.json --show-config`
- [ ] Verify loads custom config (highest priority)
- [ ] Verify custom values shown in output
- [ ] Test with invalid path - should fall back to standard search

**Setup**:
```bash
cat > /tmp/custom-test-config.json <<'EOF'
{
  "core_settings": {
    "INSTALLER_OS": "16",
    "MAX_DEFERS": 7
...
**Validation**:
```bash
sudo /Library/Management/erase-install/erase-install-defer-wrapper.sh --config=/tmp/custom-test-config.json --show-config
```

**Expected**: Shows `INSTALLER_OS: 16` and `MAX_DEFERS: 7` from custom config

#### Test 6c: --test-os-check Parameter
- [ ] Run on Mac with OS version lower than `INSTALLER_OS` target
- [ ] Without parameter: should exit due to OS check
- [ ] With `--test-os-check`: should skip OS version check and continue
- [ ] Verify log shows OS check skipped
...
#### Test 6d: Combined Parameters
- [ ] Test: `--config=/path/to/file.json --show-config`
- [ ] Test: `--config=/path/to/file.json --test-os-check`
- [ ] Test: `--show-config` with managed JSON present
- [ ] Verify all parameter combinations work correctly

---

### Test 7: Configuration Priority Order
**Goal**: Verify 4-tier configuration priority works correctly

- [ ] Create custom JSON: `/tmp/test.json` with `INSTALLER_OS=14`
- [ ] Create managed JSON: `/Library/Managed Preferences/...` with `INSTALLER_OS=15`
- [ ] Create local JSON: `/Library/Preferences/...` with `INSTALLER_OS=16`
- [ ] Test 1: Custom via `--config=/tmp/test.json` - should use `14`
- [ ] Test 2: Remove custom, should use managed `15`
- [ ] Test 3: Remove managed, should use local `16`
- [ ] Test 4: Remove local, should use script defaults

**Command**:
```bash
# Test each scenario with --show-config
...
**Goal**: Verify invalid configuration values are caught and use defaults

#### Test 8a: Invalid Numeric Values
- [ ] Create JSON with invalid `INSTALLER_OS` (e.g., "abc")
- [ ] Create JSON with negative `MAX_DEFERS` (e.g., -1)
- [ ] Run with `--show-config`
- [ ] Verify warnings logged
- [ ] Verify defaults used instead
...
**Validation**:
```bash
sudo /Library/Management/erase-install/erase-install-defer-wrapper.sh --config=/tmp/invalid-config.json --show-config 2>&1 | grep WARNING
```
...
### Test 9: Full Workflow Test with JSON Config
...
### Test 10: Jamf Configuration Profile Deployment (Optional)
...
**Before merging `v2.0-dev` → `main`, all of these must be true:**
