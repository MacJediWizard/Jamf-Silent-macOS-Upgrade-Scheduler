...
SCRIPT_VERSION="1.4.3"            # ← Bump this on every release!
...
init_plist() {
    ...
    defaults read "\$PLIST" deferCount      &>/dev/null \
        || defaults write "\$PLIST" deferCount      -int 0
    defaults read "\$PLIST" firstPromptDate &>/dev/null \
        || defaults write "\$PLIST" firstPromptDate -string "\$(date -u +%s)"
}
...
show_prompt() {
    ...
    # Build dropdown dialog with a single OK button
    args=(
        --title       "${DIALOG_TITLE}"
    ...
    case "$chosen" in
        ...
        "$DIALOG_SCHEDULE_TODAY_TEXT")
            # Ask for time selection
            subargs=( --title "Schedule Installation" --selecttitle "Select a time today to install" --selectvalues "17:00,18:00,19:00,20:00,21:00" --button1text "OK" --height 140 --width 400 --moveable --mini --ontop --json --position "${DIALOG_POSITION}" )
            subraw=$("${DIALOG_BIN}" "${subargs[@]}" 2>&1 | tee -a "${WRAPPER_LOG}")
            subcode=$?
            log_debug "swiftDialog schedule exit code: $subcode"
            log_debug "swiftDialog schedule raw output: $subraw"
